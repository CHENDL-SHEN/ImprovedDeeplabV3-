...
class Backbone(nn.Module, ABC_Model):
    def __init__(self, model_name, num_classes=20, mode='fix', segmentation=False):
        ...
        if 'resnet' in model_name:
            ...
            state_dict = torch.load("/media/ders/Fjmnew/PuzzleCAM-master/pretrained/resnet50-19c8e357.pth")
...
class DeepLabv3_Plus(Backbone):
    ...
    def forward(self, x, with_cam=False):
        ...
        x = resize_for_tensors(x, inputs.size()[2:], align_corners=True)# (4,6,512,512)align_corners=True在两个像素点之间进行插值，点之间是等距的
        # x = self.nunet(x)
